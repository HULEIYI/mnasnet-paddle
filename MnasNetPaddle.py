...
# 1.0 - tensorflow.
_BN_MOMENTUM = 1 - 0.9997  # TODO:要确定paddle是否也是！
...
class _InvertedResidual(nn.Layer):
    ...
    def __init__(
        ...
        self.layers = nn.Sequential(
            ...
            nn.BatchNorm2D(num_features=mid_ch, momentum=bn_momentum),
            nn.ReLU(),
            # Depthwise
            ...
            nn.BatchNorm2D(num_features=out_ch, momentum=bn_momentum))
...
def _stack(in_ch: int, out_ch: int, kernel_size: int, stride: int, exp_factor: int, repeats: int,
    ...
    first = _InvertedResidual(in_ch=in_ch, out_ch=out_ch, kernel_size=kernel_size, stride=stride,
                              expansion_factor=exp_factor, bn_momentum=bn_momentum)
    remaining = []
...
class MNASNet(nn.Layer):
    ...
    def __init__(
        ...
        layers = [
            ...
            nn.BatchNorm2D(num_features=depths[0], momentum=_BN_MOMENTUM),
            nn.ReLU(),
            ...
            nn.Conv2D(in_channels=depths[0], out_channels=depths[1], kernel_size=1, padding=0, stride=1,
                      bias_attr=False),
            nn.BatchNorm2D(num_features=depths[1], momentum=_BN_MOMENTUM),
            ...
            nn.BatchNorm2D(num_features=1280, momentum=_BN_MOMENTUM),
            nn.ReLU(),
        ]
    ...
    def _load_from_state_dict(self, state_dict: Dict, prefix: str, local_metadata: Dict, strict: bool,
        ...
        if version == 1 and not self.alpha == 1.0:
            ...
            v1_stem = [
                nn.Conv2D(in_channels=3, out_channels=32, kernel_size=3, padding=1, stride=2, bias_attr=False),
                nn.BatchNorm2D(num_features=32, momentum=_BN_MOMENTUM),
                nn.ReLU(),
                nn.Conv2D(in_channels=32, out_channels=32, kernel_size=3, padding=1, stride=1, groups=32,
                          bias_attr=False),
                nn.BatchNorm2D(num_features=32, momentum=_BN_MOMENTUM),
                nn.ReLU(),
                nn.Conv2D(in_channels=32, out_channels=16, kernel_size=1, padding=0, stride=1, bias=False),
                nn.BatchNorm2D(num_features=16, momentum=_BN_MOMENTUM),
